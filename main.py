...
def get_user_esp_state(user_id: int) -> dict:
    if user_id not in global_esp_states:
        global_esp_states[user_id] = {"slot_1": 0, "alert": 0}
    return global_esp_states[user_id]

# --- СТАНИ FSM (Тільки назва і час, бо комірка одна) ---
...
async def check_schedule():
    ...
    for task in tasks:
        ...
        # Сигнал для ESP на автоматичне відкриття
        state["slot_1"] = 1

        try:
            await bot.send_message(user_id, f"💊 Час приймати ліки: {pill_name}!\nАптечка автоматично відчиняється.")
        except Exception as e:
        ...
        # Плануємо перевірку через 10 хвилин, чи прийняв пацієнт ліки
        scheduler.add_job(
            check_if_taken,
            'date',
...
async def check_if_taken(sched_id: int, pill_name: str, user_id: int):
    ...
    if result and result[0] != 'прийнято':
        state = get_user_esp_state(user_id)
        state["alert"] = 1 # Вмикаємо сирену на ESP32
...
async def pill_taken(user_id: int, slot: int):
    ...
    query = '''
        ...
        WHERE id IN (
            ...
            JOIN Pills ON Schedule.pill_id = Pills.id 
            WHERE Pills.user_id = %s AND Schedule.status = 'надіслано'
        )
    '''
    execute_query(query, (user_id,), commit=True)

    try:
        await bot.send_message(user_id, f"✅ Підтверджено: ліки прийнято!", reply_markup=get_main_kb())
    except Exception as e:
...
async def cmd_start(message: types.Message):
    ...
    if len(args) > 1 and args[1].startswith("patient_"):
        try:
            ...
            await message.answer("✅ Ви успішно підписалися на сповіщення пацієнта!", reply_markup=get_main_kb())
            try:
                await bot.send_message(patient_id, "👨‍👩‍👧‍👦 Ваш родич щойно підключився до сповіщень!")
            except Exception:
                pass
            return
        except Exception as e:
...
async def generate_relative_link(message: types.Message):
    ...
    text = f"🔗 Надішліть це посилання родичу:\n\n{link}"
    await message.answer(text)

@dp.message(F.text == "💊 Мої ліки")
async def view_pills(message: types.Message):
    query = '''
        SELECT Pills.id, Pills.name, Schedule.time_to_take, Schedule.status 
        FROM Pills 
...
async def save_new_time(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("✅ Час прийому змінено!", reply_markup=get_main_kb())
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await state.update_data(pill_name=message.text.strip(), slot_number=1)
...
async def process_time(message: types.Message, state: FSMContext):
    ...
    pill_id = execute_query(pill_query, (message.chat.id, data['pill_name'], data['slot_number']), fetchone=True, commit=True)[0]
