...
def xor_decrypt(data_b64: str, key: str) -> str:
    ...
    kb = key.encode("utf-8")
    try:
        db = base64.b64decode(data_b64)
        return bytes(b ^ kb[i % len(kb)] for i, b in enumerate(db)).decode("utf-8")
    except Exception:
        return data_b64


TERMS_TEXT = (
    "📜 <b>Політика конфіденційності та Умови використання PillBox</b>\n\n"
    "Використовуючи бот PillBox, ви погоджуєтесь з наступним:\n\n"
    "1️⃣ <b>Збір даних:</b> Ми зберігаємо ваш Telegram ID, ім'я, "
    "розклад прийому ліків та історію подій для коректної роботи сервісу.\n\n"
    "2️⃣ <b>Використання:</b> Дані використовуються виключно для "
    "нагадувань про прийом ліків та синхронізації з пристроєм ESP32.\n\n"
    "3️⃣ <b>Захист:</b> Всі персональні та медичні дані (ім'я, назви ліків, дозування, розклад) "
    "надійно шифруються перед збереженням у базу даних.\n\n"
    "4️⃣ <b>Видалення:</b> Ви можете будь-коли запросити повне "
    "видалення ваших даних, написавши /delete_my_data.\n\n"
    "Натисніть кнопку нижче, щоб прийняти умови та почати користуватися ботом."
)
...
async def ensure_user(telegram_id: int, name: str):
    async with pool.acquire() as c:
        await c.execute(
            ...
            telegram_id, xor_encrypt(name, API_SECRET),
        )


async def get_user(telegram_id: int):
    async with pool.acquire() as c:
        row = await c.fetchrow("SELECT * FROM users WHERE telegram_id=$1", telegram_id)
        if row:
            d = dict(row)
            d["name"] = xor_decrypt(d["name"], API_SECRET) if d["name"] else d["name"]
            return d
        return row
...
async def get_pills(user_id: int):
    async with pool.acquire() as c:
        ...
        for r in rows:
            d = dict(r)
            d["name"] = xor_decrypt(d["name"], API_SECRET)
            if d["dosage"]:
                d["dosage"] = xor_decrypt(d["dosage"], API_SECRET)
            result.append(d)
        return result
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if not user["accepted_terms"]:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="✅ Згоден / I Agree", callback_data="accept_terms")]
            ]
        )
...
async def accept_terms_cb(callback: types.CallbackQuery):
    async with pool.acquire() as c:
        await c.execute(
            "UPDATE users SET accepted_terms = TRUE WHERE telegram_id=$1",
            callback.from_user.id,
        )
    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer(
        f"✅ Дякуємо! Умови прийнято.\n\n"
        f"👋 Вітаю, {callback.from_user.first_name}!\n\n"
...
async def my_pills(message: types.Message):
    ...
    async with pool.acquire() as c:
        for p in pills:
            ...
            times_str = ", ".join(json.loads(xor_decrypt(s["times"], API_SECRET))) if s else "—"
    ...
    async with pool.acquire() as c:
        if field == "times":
            ...
            await c.execute(
                ...
                xor_encrypt(json.dumps(times), API_SECRET), pill_id,
            )
        elif field == "count":
            if not message.text.strip().isdigit():
                return await message.answer("⚠️ Введіть ціле число:")
            ...
            await c.execute(
                ...
                xor_encrypt(message.text.strip(), API_SECRET), pill_id,
            )
    await state.clear()
    await message.answer(
...
async def show_schedule(message: types.Message):
    ...
    async with pool.acquire() as c:
        for p in pills:
            ...
            if not s:
                continue
            times = ", ".join(json.loads(xor_decrypt(s["times"], API_SECRET)))
            days  = ", ".join(DAYS_UA[d] for d in sorted(json.loads(xor_decrypt(s["days"], API_SECRET))))
            text += (
                f"🔹 <b>{p['name']}</b> {p['dosage']}\n"
                f"  ⏰ {times}\n"
                f"  📆 {days}\n\n"
...
async def generate_weekly_instruction(message: types.Message):
    ...
    for day_idx in range(7):
        ...
        for p in pills:
            ...
            if day_idx in days:
                times = ", ".join(json.loads(xor_decrypt(p["times"], API_SECRET)))
                name = xor_decrypt(p["name"], API_SECRET)
                dosage = xor_decrypt(p["dosage"], API_SECRET)
                day_pills.append(f"  💊 {name} ({dosage}) — на {times}")
...
async def cmd_history(message: types.Message):
    ...
    for l in logs:
        ...
        name = xor_decrypt(l["name"], API_SECRET) if l["name"] else "—"
...
async def lifespan(app: FastAPI):
    ...
    scheduler.add_job(check_esp_status, "interval", minutes=10)
    scheduler.start()
    ...
    async with pool.acquire() as c:
        ...
        for p in pills:
            ...
            schedule.append({
                ...
                "name":   xor_decrypt(p["name"], API_SECRET),
                "dosage": xor_decrypt(p["dosage"], API_SECRET),
                "days":   json.loads(xor_decrypt(s["days"], API_SECRET)),
            })
...
async def check_reminders():
    ...
    for r in rows:
        days = json.loads(xor_decrypt(r["days"], API_SECRET))
        times = json.loads(xor_decrypt(r["times"], API_SECRET))
        if current_day in days and current_time in times:
            try:
...
async def check_esp_status(): #раз на 10хв перевіряємо чи відвалилась еспшка
    ...
    async with pool.acquire() as c:
        users = await c.fetch(
            "SELECT telegram_id, device_id, last_ping FROM users "
            "WHERE device_id IS NOT NULL AND last_ping IS NOT NULL "
            "AND last_ping < $1 AND (esp_notified IS NULL OR esp_notified = FALSE)",
            threshold,
        )
        for u in users:
            try:
                await bot.send_message(
                    u["telegram_id"],
                    "⚠️ <b>Зв'язок з ESP32 втрачено.</b>\n"
                    "Пристрій не відповідає понад 20 хвилин.\n\n"
                    f"🔌 Пристрій: <code>{u['device_id']}</code>\n"
                    "Перевірте живлення та Wi-Fi підключення.",
                    parse_mode="HTML",
                )
                await c.execute(
                    "UPDATE users SET esp_notified = TRUE WHERE telegram_id=$1",
                    u["telegram_id"],
                )
            except Exception:
                pass
